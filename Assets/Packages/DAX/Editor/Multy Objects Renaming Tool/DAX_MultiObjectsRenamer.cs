...
{
	const string MenuItemSTR = "Tools/Others/Multi objects rename tool %#&n";
	const string TabCaptionSTR = "MO Rename";

	Object[] foundObjects;
	bool[] exportedObjects;

	bool tOnlyROOT = false;
	...
	void updateInfo (bool loadSelection = false)
	{
		try
		{
			foundObjects = new Object[0];
			if (loadSelection)
			{
				foundObjects = Selection.objects;
			}
			else
			{
				foundObjects = GameObject.FindObjectsOfType(typeof(GameObject));
			}
			...
			if (foundObjects != null)
			{
				...
				for (int i = 0; i < foundObjects.Length; i++)
				{
					...
					else if ((foundObjects[i] as GameObject).transform.parent == null)
					{
						maxSize++;
					}
				}
				...
				for (int i = 0; i < foundObjects.Length; i++)
				{
					if (!tOnlyROOT | ((foundObjects[i] as GameObject).transform.parent == null))
					{
						tfObj[curItem] = foundObjects[i];
						curItem++;
					}
				}
				foundObjects = tfObj;

				exportedObjects = new bool[foundObjects.Length];
				for (int i = 0; i < foundObjects.Length; i++)
				{
	...
	void OnGUI ()
	{
		try
		{
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField("Add prefix:", GUILayout.MaxWidth(75.0f));
			PrefixToAdd = EditorGUILayout.TextField(PrefixToAdd);
			EditorGUILayout.EndHorizontal();

			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField("Add suffix:", GUILayout.MaxWidth(75.0f));
			SuffixToAdd = EditorGUILayout.TextField(SuffixToAdd);
			EditorGUILayout.EndHorizontal();
					...
					if (foundObjects[i] != null)
					{
						try
						{
							...
							try
							{
								...
								{
									exportedObjects[i] = EditorGUILayout.BeginToggleGroup(PrefixToAdd + foundObjects[i].name.Replace(ReplaceFrom, ReplaceTo) + SuffixToAdd, exportedObjects[i]); //checkboxes left
									EditorGUILayout.EndToggleGroup();
								}
								else if ((PrefixToAdd != "" || SuffixToAdd != "") & tShowResult)
								{
									exportedObjects[i] = EditorGUILayout.BeginToggleGroup(PrefixToAdd + foundObjects[i].name + SuffixToAdd, exportedObjects[i]); //checkboxes left
									EditorGUILayout.EndToggleGroup();
								}
								else
								{
									exportedObjects[i] = EditorGUILayout.BeginToggleGroup(foundObjects[i].name, exportedObjects[i]); //checkboxes left
									EditorGUILayout.EndToggleGroup();
								}
							}
							catch { };
					...
					for (int i = 0; i < foundObjects.Length; i++)
					{
						if (foundObjects[i] != null)
						{
							try
							{
								...
								{
									foundObjects[i].name = PrefixToAdd + foundObjects[i].name.Replace(ReplaceFrom, ReplaceTo) + SuffixToAdd;
								}
								else if ((PrefixToAdd != "" || SuffixToAdd != "") & exportedObjects[i])
								{
									foundObjects[i].name = PrefixToAdd + foundObjects[i].name + SuffixToAdd;
								}
							}
							catch { };
						}
